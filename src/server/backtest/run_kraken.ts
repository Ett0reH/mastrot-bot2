...
async function fetch15mData(
  ...
  if (fs.existsSync(cacheFilePath)) {
    try {
      ...
      if (cachedBars && cachedBars.length > 0) {
         allBars = cachedBars.filter((b: Bar) => {
           const t = new Date(b.t).getTime();
           return t >= tFetchStart && t <= tEnd;
         });
         console.log(`Cache fully loaded offline for ${symbol} covering the range (${allBars.length} bars).`);
         return allBars;
      }
    } catch (e) {
  ...
  console.log(`Downloading ${symbol} [15Min] from Kraken (warmup start: ${new Date(tFetchStart).toISOString()} to ${end})...`);
  ...
  while (currentSince < tEnd) {
      try {
          const limit = 1000;
          const ohlcv = await exchange.fetchOHLCV(symbol, '15m', currentSince, limit);
          ...
          for (const c of ohlcv) {
             const t = c[0];
             if (t > tEnd) continue;
             if (t < currentSince) continue; // ccxt sometimes returns older data
          ...
          fetchCount++;
          if (fetchCount % 10 === 0) {
              console.log(`Fetched up to ${new Date(currentSince).toISOString()} for ${symbol} (${allBars.length} bars)`);
          }

          // if we reached the end or fetched less than limit
...
async function runEventDrivenBacktest() {
  ...
  console.log("--- KRAKEN DATA BACKTEST ---");
  ...
  for (let i = 0; i < maxTicks; i++) {
    ...
    for (const symbol of SYMBOLS) {
      ...
      if (!position && h1Closed && features && !capitalHealth.isHalted) {
        ...
        if (symbolStates["BTC/USD:USD"] && symbolStates["BTC/USD:USD"][i] && symbolStates["BTC/USD:USD"][i].features) {
          btcTrend1H = symbolStates["BTC/USD:USD"][i].features.trend1H;
          btcRegime = symbolStates["BTC/USD:USD"][i].regime;
        }

        const signal = SignalLayer.evaluate(features, regime, symbol, { btcTrend1H, btcRegime });

        if (symbol === "DOGE/USD:USD" && regime === "EUPHORIA") {
        ...
        if (signal.direction !== "NEUTRAL") {
          ...
          if (gate.allowed) {
            ...
            if (tierDecision.blocked || tierDecision.tierLabel === "TRANSITION_BLOCKED") {
                if (symbol === "DOGE/USD:USD" && regime === "EUPHORIA" && gate.allowed) {
            ...
            } else if (symbol === "DOGE/USD:USD" && regime === "EUPHORIA" && gate.allowed) {
      ...
      if (FEATURE_FLAGS.enableTradeBudgetPerCandle) {
        ...
        for (const candidate of candidateSignals) {
           if (executedThisCandle < maxNewPositionsPerCandle) {
               ...
               if (candidate.symbol === "DOGE/USD:USD") {
